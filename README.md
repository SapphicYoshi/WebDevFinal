...
3. **CSS Grid** - Creates responsive layouts for navigation and card controls. Located in `.parent` and `.bottom-nav` rules in `css/style.css`. Provides flexible, adaptive positioning.

4. **CSS 3D Transforms** - Implements card flip animations for flashcards. Located in `.div1.flipped` and `.card-back` rules in `css/style.css`. Adds engaging visual feedback for studying.
...
Overall, integrating stretch concepts like localStorage and the Audio API deepened my understanding of browser APIs. The theming system was particularly satisfying, as it made the site feel polished and personalized. This project reinforced that clean architecture and user experience go hand-in-hand. 
